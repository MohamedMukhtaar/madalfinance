...
export const create = (conn, data) => {
  const { payment_number, customer_id, payment_date, payment_method, reference_number, amount, notes, received_by } = data;
  return run(
    conn,
    `INSERT INTO payments (payment_number, customer_id, payment_date, payment_method, reference_number, amount, notes, received_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [payment_number, customer_id, payment_date, payment_method, reference_number ?? null, amount, notes ?? null, received_by]
  ).then((r) => r.insertId);
};
