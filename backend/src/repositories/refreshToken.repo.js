...
export const findValid = (conn, tokenHash) =>
  run(
    ...
    `SELECT t.*, u.user_id, u.username, u.full_name, u.status, r.role_name AS role
      ...
      WHERE t.token_hash = ? AND t.is_revoked = 0 AND t.expires_at > NOW()`,
    [tokenHash]
  ).then((rows) => rows[0]);

export const revoke = (conn, tokenId) =>
  run(conn, `UPDATE refresh_tokens SET is_revoked = 1, revoked_at = NOW() WHERE token_id = ?`, [tokenId]);

export const revokeAllForUser = (conn, userId) =>
  run(conn, `UPDATE refresh_tokens SET is_revoked = 1, revoked_at = NOW() WHERE user_id = ?`, [userId]);

export const cleanupExpired = (conn) =>
  run(conn, `DELETE FROM refresh_tokens WHERE expires_at < DATE_SUB(NOW(), INTERVAL 7 DAY)`);
