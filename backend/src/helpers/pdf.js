...
const addCompanyHeader = (doc, settings) => {
  ...
  if (logo) {
    const { width, height } = fitSize(logo, right - left, 44);
  ...
  doc
    ...
    .fontSize(logo ? 14 : 18)
    .font('Helvetica-Bold')
    .text(companyName(settings), left, y, { width: right - left, align: 'left' });
  const contact = companyContact(settings);
  if (contact) {
...
const addPartyCard = (doc, title, name, lines = []) => {
  ...
  const y = doc.y + 6;
  const boxHeight = 48 + lines.length * 12;
  doc.roundedRect(48, y, 509, boxHeight, 8).stroke('#e2e8f0');
...
export const generateInvoicePdf = async ({ invoice, items, customer, settings }) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.fontSize(8).fillColor('#94a3b8').text(
      `Generated by ${companyName(settings)} · ${dayjs().format('DD MMM YYYY HH:mm')}`,
      { align: 'center' }
    );
...
export const generateIncomeStatementPdf = async ({ data, fromDate, toDate, settings }) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    doc.moveDown(1);
    const summaryText = `Income of ${money(settings, totalIncome)} minus expenses of ${money(settings, totalExpense)} results in a ${isProfit ? 'profit' : 'loss'} of ${money(settings, Math.abs(netProfit))} (${marginPct}% margin) for ${periodLabel}.`;
    doc.fontSize(9).font('Helvetica');
    const summaryH = doc.heightOfString(summaryText, { width: 489, lineGap: 2 });
    const boxY = doc.y;
    doc.roundedRect(48, boxY, 509, summaryH + 36, 8).fillAndStroke('#f8fafc', '#e2e8f0');
    doc.fontSize(8).font('Helvetica-Bold').fillColor('#94a3b8').text('SUMMARY', 58, boxY + 10);
    doc.fontSize(9).font('Helvetica').fillColor('#475569').text(summaryText, 58, boxY + 24, { width: 489, lineGap: 2 });
    doc.y = boxY + summaryH + 44;
...
export const generateMemberStatementPdf = async ({ statement, fromDate, toDate, settings }) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    const tableTop = doc.y + 4;
    const colX = { date: 54, time: 106, type: 148, ref: 202, due: 330, paid: 388, loan: 446, balance: 496 };
    const drawHeader = (y) => {
      doc.rect(48, y, 509, 20).fill('#101848');
      doc.fontSize(8).font('Helvetica-Bold').fillColor('#ffffff');
      doc.text('DATE', colX.date, y + 6, { width: 50 });
      doc.text('TIME', colX.time, y + 6, { width: 40 });
      doc.text('TYPE', colX.type, y + 6, { width: 52 });
      doc.text('REFERENCE', colX.ref, y + 6, { width: 124 });
      doc.text('DEBIT', colX.due, y + 6, { width: 54, align: 'right' });
      doc.text('CREDIT', colX.paid, y + 6, { width: 54, align: 'right' });
      doc.text('LOAN', colX.loan, y + 6, { width: 46, align: 'right' });
      doc.text('BALANCE', colX.balance, y + 6, { width: 55, align: 'right' });
    };
    ...
    if (!rows.length) {
      ...
      rows.forEach((row, i) => {
        ...
        if (i % 2 === 1) doc.rect(48, y, 509, 20).fill('#f8fafc');
        doc.fontSize(8.5).font('Helvetica').fillColor('#475569').text(row.date, colX.date, y + 5, { width: 50, lineBreak: false });
        doc.text(row.time, colX.time, y + 5, { width: 40, lineBreak: false });
        doc.font('Helvetica-Bold').fillColor('#0f172a').text(row.type || '—', colX.type, y + 5, { width: 52, lineBreak: false });
        doc.font('Helvetica').fillColor('#475569').text(row.reference || row.description || '—', colX.ref, y + 5, { width: 124, lineBreak: false, ellipsis: true });
        ...
        doc.fillColor('#ea580c').text(row.loan > 0 ? money(settings, row.loan) : '—', colX.loan, y + 5, { width: 46, align: 'right' });
        const balanceLabel = money(settings, row.balance);
        doc.font('Helvetica-Bold').fillColor('#0f172a').text(balanceLabel, colX.balance, y + 5, { width: 55, align: 'right' });
        y += 22;
      });
    }
    doc.y = y + 10;

    const summaryText = `${member.member_name || 'Member'} was charged ${money(settings, charged)} in dues, received ${money(settings, loans)} in loans, paid ${money(settings, paid)} back, with ${money(settings, outstanding)} dues and ${money(settings, loanBalance)} loan still outstanding. Net balance: ${money(settings, closingBalance)}.`;
    doc.fontSize(9).font('Helvetica');
    const summaryH = doc.heightOfString(summaryText, { width: 489, lineGap: 2 });
    const boxY = doc.y;
    doc.roundedRect(48, boxY, 509, summaryH + 36, 8).fillAndStroke('#f8fafc', '#e2e8f0');
...
/** Generic tabular report: branded header, auto-sized columns, repeated table header on each page. */
export const generateReportPdf = async ({ title, subtitle, columns, rows, settings }) => {
  ...
  await new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 48, layout: columns.length > 6 ? 'landscape' : 'portrait' });
    const stream = fs.createWriteStream(filePath);
    doc.pipe(stream);
    ...
    addCompanyHeader(doc, settings);
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#0f172a').text(title, left, doc.y);
    doc.fontSize(9).font('Helvetica').fillColor('#64748b')
      .text(subtitle || `Generated ${dayjs().format('DD MMM YYYY HH:mm')}`, left, doc.y + 2);
    doc.moveDown(1);

    const weights = columns.map((c) => c.width || 20);
    const totalWeight = weights.reduce((sum, w) => sum + w, 0);
    const widths = weights.map((w) => (w / totalWeight) * tableWidth);
    const xs = widths.map((_, i) => left + widths.slice(0, i).reduce((sum, w) => sum + w, 0));
    const numericCol = columns.map((c) => rows.length > 0 && rows.every((r) => r[c.key] == null || r[c.key] === '' || typeof r[c.key] === 'number'));

    const drawHeader = (y) => {
      doc.fontSize(8).font('Helvetica-Bold');
      const h = Math.max(...columns.map((c, i) => doc.heightOfString(String(c.header).toUpperCase(), { width: widths[i] - pad * 2 }))) + pad * 2;
      doc.rect(left, y, tableWidth, h).fill('#101848');
      columns.forEach((c, i) => {
        doc.fillColor('#ffffff').text(String(c.header).toUpperCase(), xs[i] + pad, y + pad, {
    ...
    if (!rows.length) {
      doc.fontSize(9).font('Helvetica').fillColor('#94a3b8').text('No records.', left + pad, y + 8);
      y += 28;
    }
    rows.forEach((row, r) => {
      doc.fontSize(8.5).font('Helvetica');
      const texts = columns.map((c) => cellText(row[c.key]));
      const h = Math.max(...texts.map((t, i) => doc.heightOfString(t, { width: widths[i] - pad * 2 }))) + pad * 2;
      if (y + h > bottom) {
        doc.addPage();
        y = drawHeader(48);
        doc.fontSize(8.5).font('Helvetica');
      }
      if (r % 2 === 1) doc.rect(left, y, tableWidth, h).fill('#f8fafc');
      texts.forEach((t, i) => {
        doc.fillColor('#0f172a').text(t, xs[i] + pad, y + pad, {
          width: widths[i] - pad * 2,
          align: numericCol[i] ? 'right' : 'left',
        });
      });
    ...
    doc.moveTo(left, y).lineTo(left + tableWidth, y).strokeColor('#e2e8f0').stroke();
    doc.fontSize(8).font('Helvetica').fillColor('#94a3b8')
      .text(`Generated by ${companyName(settings)} · ${dayjs().format('DD MMM YYYY HH:mm')}`, left, y + 14, {
        width: tableWidth,
        align: 'center',
      });
